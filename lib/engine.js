(function (root) {
  ...
  async function simplifyRecord(record, settings, capability) {
    ...
    if (settings.useOpenRouter) {
      simplified = await simplifyWithOpenRouter(record.text, settings);
    }
    if (!simplified && settings.useAI && capability.state === "ready") {
