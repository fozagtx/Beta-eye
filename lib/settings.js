(function (root) {
  ...
  async function getSettings() {
    const data = await chrome.storage.sync.get(["redacto:settings"]);
    const settings = migrateSettings(data["redacto:settings"]);
    if (!data["redacto:settings"] || data["redacto:settings"].schemaVersion !== CURRENT_SCHEMA_VERSION) {
      await chrome.storage.sync.set({ "redacto:settings": settings });
    }
    return settings;
  }
  ...
  async function saveSettings(partial) {
    ...
    await chrome.storage.sync.set({ "redacto:settings": next });
    return next;
  }

  async function getOnboardingState() {
    const data = await chrome.storage.local.get(["redacto:onboarded"]);
    return Boolean(data["redacto:onboarded"]);
  }

  async function setOnboardingState(onboarded) {
    await chrome.storage.local.set({ "redacto:onboarded": Boolean(onboarded) });
  }
  ...
  if (typeof module !== "undefined") module.exports = root.RedactoSettings;
