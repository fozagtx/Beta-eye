(function (root) {
  ...
  function simplify(text, settings) {
    ...
    let output = text.replace(PROTECTED_PATTERN, (match) => {
      const token = `__BETA_EYE_PROTECTED_${protectedValues.length}__`;
    ...
    protectedValues.forEach((value, index) => {
      output = output.replace(`__BETA_EYE_PROTECTED_${index}__`, value);
    });
    return output;
  }
  ...
  function shouldPreserveBlock(text) {
    return /\b(warning|disclaimer|dosage|prescription|liability|investment|diagnosis)\b/i.test(
      text,
    );
  }

  root.BetaEyeRulesFallback = { simplify, shouldPreserveBlock };
  if (typeof module !== "undefined") module.exports = root.BetaEyeRulesFallback;
