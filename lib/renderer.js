(function (root) {
  ...
  function renderChangeView(element, original, simplified) {
    ...
    for (const word of words) {
      if (/^\s+$/.test(word) || originalWords.has(word.toLowerCase())) {
        ...
        marker.className = "see-change-token";
  ...
  function applyDisplay(settings) {
    ...
    style.textContent = `${fontFace}
      ...
      .see-sentence-pacing .see-simplified-text{margin-block:1.1em;}
    ...
    document.documentElement.classList.toggle(
      "see-theme-highContrast",
      display.theme === "highContrast",
    );
    document.documentElement.classList.toggle("see-theme-soft", display.theme === "soft");
  }

  root.SeeRenderer = { state, render, restoreAll, toggleOriginal, applyDisplay, renderChangeView };
